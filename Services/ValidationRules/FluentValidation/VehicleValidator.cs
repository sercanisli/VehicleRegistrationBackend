    ...
    public class VehicleValidator : AbstractValidator<Vehicle>
    {
        public VehicleValidator()
        {
            ...
            RuleFor(v => v.Plate).NotEmpty().NotNull().WithMessage("VehicleTypeId cannot be empty or null")
                ...
                .Matches(@"^\d{2}(?=.*[A-Za-z])[A-Za-z]{1,3}\d{2}$")
                .WithMessage("Plate number invalid format. First two characters must be numbers, next character(s) must be letters, last two characters must be numbers.");
